...
class TestSupportSpinorMachine(unittest.TestCase):
    ...
    def test_kernel(self):
        ...
        K = model._compute_kernel(X_train, X_test)
        self.assertEqual(K.shape, (X_test.shape[0], X_train.shape[0]))
    ...
    def test_modulation(self):
        ...
        model = SupportSpinorMachine(C=1, gamma=1e-3, mu=1)
        K = model._compute_kernel(X_train, X_test)
        K_mod = model._modulate_kernel(K)
        self.assertEqual(K_mod.shape, K.shape)
